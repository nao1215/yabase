...
pub fn bech32_roundtrip_test() {
  let data = <<0, 1, 2, 3, 4>>
  let assert Ok(encoded) = bech32.encode("test", data)
...
pub fn bech32_empty_data_test() {
  let assert Ok(encoded) = bech32.encode("a", <<>>)
  let assert Ok(decoded) = bech32.decode(encoded)
...
pub fn bech32m_roundtrip_test() {
  let data = <<0, 1, 2, 3>>
  let assert Ok(encoded) = bech32.encode_m("test", data)
  let assert Ok(decoded) = bech32.decode(encoded)
  assert decoded.hrp == "test"
...
pub fn bech32m_empty_data_test() {
  let assert Ok(encoded) = bech32.encode_m("a", <<>>)
  let assert Ok(decoded) = bech32.decode(encoded)
  assert decoded.hrp == "a"
  assert decoded.variant == Bech32mV
}
...
pub fn auto_detect_bech32m_test() {
  let data = <<10, 20>>
  let assert Ok(encoded) = bech32.encode_m("bc", data)
  let assert Ok(decoded) = bech32.decode(encoded)
  assert decoded.variant == Bech32mV
}
...
pub fn decode_invalid_checksum_test() {
  let assert Ok(encoded) = bech32.encode("test", <<1, 2, 3>>)
...
pub fn encode_hrp_too_long_test() {
  ...
  assert case bech32.encode(long_hrp, <<1>>) {
...
pub fn encode_result_overlength_test() {
  ...
  >>
  assert case bech32.encode("bc", big_data) {
...
pub fn padding_bits_preserved_on_roundtrip_test() {
  ...
  let assert Ok(encoded) = bech32.encode("test", <<0xff>>)
  let assert Ok(decoded) = bech32.decode(encoded)
...
pub fn bech32m_tb_prefix_roundtrip_test() {
  let data = <<1, 2, 3>>
  let assert Ok(encoded) = bech32.encode_m("tb", data)
  let assert Ok(decoded) = bech32.decode(encoded)
  assert decoded.variant == Bech32mV
...
pub fn bech32m_long_hrp_test() {
  // 10 char HRP
  let data = <<42>>
  let assert Ok(encoded) = bech32.encode_m("abcdefghij", data)
  let assert Ok(decoded) = bech32.decode(encoded)
