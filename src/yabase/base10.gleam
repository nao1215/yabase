...
fn encode_int(num: Int, acc: String) -> String {
  case num {
    ...
    _ -> {
      ...
      let char = string_char_at("0123456789", remainder)
      encode_int(num / 10, char <> acc)
    }
  }
}
...
fn string_to_int(input: String, acc: Int, pos: Int) -> Result(Int, CodecError) {
  case string.pop_grapheme(input) {
    ...
    Ok(#(c, rest)) ->
      case c {
        "0" -> string_to_int(rest, acc * 10 + 0, pos + 1)
        "1" -> string_to_int(rest, acc * 10 + 1, pos + 1)
        "2" -> string_to_int(rest, acc * 10 + 2, pos + 1)
        "3" -> string_to_int(rest, acc * 10 + 3, pos + 1)
        "4" -> string_to_int(rest, acc * 10 + 4, pos + 1)
        "5" -> string_to_int(rest, acc * 10 + 5, pos + 1)
        "6" -> string_to_int(rest, acc * 10 + 6, pos + 1)
        "7" -> string_to_int(rest, acc * 10 + 7, pos + 1)
        "8" -> string_to_int(rest, acc * 10 + 8, pos + 1)
        "9" -> string_to_int(rest, acc * 10 + 9, pos + 1)
